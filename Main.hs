module Main where
    ...
    -- data to mark the play, instructions and exitgame option
    data Menu_Input = Play | Instructions | Exit_Game

    main = do
           hSetBuffering stdin NoBuffering -- we don't want the IO buffering
    ...
    -- links to the new_eventloop in EventLoop module
    play_handle = do
